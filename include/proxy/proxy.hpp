 ...
 */
typedef map<int,int > interface_map; //map< interface, proxyinstance{index}>
...
/**
 * @brief If some proxy instance in the timeout time dont react to an debug message it will be ignored.
 */
...
private:
     ...
     vector<string> m_restore_rp_filter_vector; //save interface who musst set to true after end
     ...
     //map< instance, map < interface, vector< group_state> > > state_table
     ...
     bool init_mrt_socket(); //create ipv4/ipv6 socket and set mrt-flag
