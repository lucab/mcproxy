...
proxy_instance::proxy_instance():
    worker(PROXY_INSTANCE_MSG_QUEUE_SIZE), m_upstream(0), m_addr_family(-1), m_version(-1)
{
    HC_LOG_TRACE("");
...
bool proxy_instance::init(int addr_family, int version, int upstream_index, int upstream_vif, int downstream_index, int downstram_vif,receiver* r){
    HC_LOG_TRACE("");
    ...
    m_check_source.init(m_addr_family);

    m_receiver = r;

    m_routing = routing::getInstance();
...
/*void proxy_instance::split_traffic(int if_index, addr_storage g_addr){
     HC_LOG_TRACE("");
     cout << "start set_mc_routen" << endl;
     cout << "if_index:" << if_index << endl;

     proxy_msg msg;
     src_state_map* tmp_src_state_map;

     upstream_src_state_map::iterator iter_uss_map;
     state_table_map::iterator iter_table;
     g_state_map::iterator iter_state;
     src_state_map::iterator iter_src_state;
     ...
     if(vif_list.size() == 0) return; //if nobody join this group ignore

     cout << "founded vifs:";
     for( list<int>::iterator it = vif_list.begin(); it != vif_list.end(); it++){
          cout << *it << " ";
     }
     cout << endl; //???????????????????????????????
     ...
     if(if_index == m_upstream){
          iter_uss_map = m_upstream_state.find(g_addr);
          if(iter_uss_map == m_upstream_state.end()){
               HC_LOG_ERROR("cant find g_addr:" << g_addr << " for upstream if_index:" << if_index);
               return;
          }
          tmp_src_state_map = &iter_uss_map->second;
     }else{ //downstream
          iter_table = m_state_table.find(if_index);
          if(iter_table == m_state_table.end()){
               HC_LOG_ERROR("cant find downstream if_index:" << if_index);
               return;
          }
          iter_state =  iter_table->second.find(g_addr);
          if(iter_state == iter_table->second.end()){
               HC_LOG_ERROR("cant find g_addr:" << g_addr << " for downstream if_index:" << if_index);
               return;
          }

          tmp_src_state_map = &iter_state->second.first;
     }

     cout << "foundet source:";

     msg.type = proxy_msg::ROUTING_MSG;

     //add a route for every source
     for(iter_src_state = tmp_src_state_map->begin(); iter_src_state != tmp_src_state_map->end(); iter_src_state++){
          cout << iter_src_state->first << " ";
          msg.msg = new routing_msg(routing_msg::ADD_ROUTE, vif, iter_src_state->first , g_addr, vif_list);
          m_routing->add_msg(msg);
...
bool proxy_instance::split_traffic(int if_index, const addr_storage& g_addr, const addr_storage& src_addr){
    ...
    msg.type = proxy_msg::ROUTING_MSG;
    msg.msg = new routing_msg(routing_msg::ADD_ROUTE, vif, g_addr, src_addr, vif_list);
    m_routing->add_msg(msg);
...
bool proxy_instance::del_route(int if_index, const addr_storage& g_addr, const addr_storage& src_addr){
    ...
    proxy_msg msg;
    msg.type = proxy_msg::ROUTING_MSG;
    msg.msg = new routing_msg(routing_msg::DEL_ROUTE, vif, g_addr, src_addr);
    m_routing->add_msg(msg);
...
void proxy_instance::registrate_if(int if_index){
    ...
    //##-- routing --##
    proxy_msg m;
    m.type = proxy_msg::ROUTING_MSG;
    m.msg = new struct routing_msg(routing_msg::ADD_VIF, if_index, vif);
    m_routing->add_msg(m);
...
void proxy_instance::unregistrate_if(int if_index){
    ...
    proxy_msg m;
    m.type = proxy_msg::ROUTING_MSG;
    m.msg = new struct routing_msg(routing_msg::DEL_VIF,if_index, vif);
    m_routing->add_msg(m);
